...
#Jan 14: Fixed BUG in hourly_average function, sped it up by a factor of 7!
#Jan 20: Cleaned up the look!
...
COImporter <- function(directory) {
  ...
  master$Date_Time <- as.POSIXct(strptime(master$Date_Time, format = "%m/%d/%Y %H:%M:%S", tz = "GMT"))
  # Change ppm to ppb
...
DataFixer <- function(master, site)
{
  ...
  positives <- positives[master$C0ppb[positives + 3] >= 800 &
                         master$C0ppb[positives + 3] < 2000]
  ...
  for (i in 1 : length(positives)) {
    if (positives[i] != endcalib[i]) {
      if (sum(abs(master$C0ppb[(positives[i] + 1):endcalib[i]]-
              master$C0ppb[positives[i] : (endcalib[i] - 1)])) < 250) {
  ...
  check2 <- 0 * (1 : (N - 5))
  check2 <- master[5 : length(master[, 1]), 2] -
            master[1 : (length(master[, 1]) - 4), 2]
  ...
  pb <- tkProgressBar("Marking Self-checks", "% complete", 0, N1, 
						initial = 0)
  while(i <= N1){
	
    master$notes[truths[i] : (truths[i] + 20)][
                (master[truths[i] : (truths[i] + 20), 2] == master[truths[i], 2])
                & master[truths[i] : (truths[i] + 20), 3] == 0] <- 2
    j <- truths[i]
    while(truths[i] < 25 + j & i <= N1) {
      i <- i + 1
    }
    info <- sprintf("%d%% done", round(100*i/N1))
  ...
  # Temporary fix to get rid of DST problems!
  master <- master[duplicated(master[,1]) == FALSE,]
  #Write the new table to a file#
  write.table(ExcelFormat(master),
              file = paste(site, "CO", max(as.Date(master[,1])),
                           ".csv", sep = ""), sep = ",",
                           row.names = FALSE)
  write.table(master[master[,3] == 1, ],
              file = paste(site, "CO_Precision_Checks",
                           max(as.Date(master[,1])), ".csv", sep = ""),
                           sep = ",", row.names = FALSE)
...
ExcelFormat <- function(dataframe) {
  separated <- colsplit(as.character(dataframe[, 1]),
                        split = " ",
                        names = c("DATE", "TIME"))
...
COHourlyAverage <- function(dataframe, site) {
  library(xts)
  ...
  ends <- endpoints(dataframe[, 1], "hours")

  times2 <- as.POSIXct(round(dataframe[, 1][ends[2 : length(ends)]] - 1800,
                             "hour"), tz = "GMT")+1
  ind <- which(times %in% times2)
  N <- length(times)
  N2 <- length(ends)
  ...
  cat("\nAveraging data for", N, "hours\n", sep = " ")
      i <- 1
	  pb <- tkProgressBar("Averaging Data", "% complete", 0, (N2 - 1), 
						initial = 0)
      while (i <= N2 - 1) {
          k <- dataframe[(ends[i] + 1):ends[i + 1], ]
          w <- k[k[, 3] == 0, ]
          if (nrow(w) != 0) {
            Other[ind[i]] <- nrow(k)-nrow(w)
            CO_sd[ind[i]] <- sd(w[, 2])
            Number[ind[i]] <- length(w[, 1])
            Center[ind[i]] <- mean(w[, 1])

            hourly[ind[i], 2] <- mean(w[, 2])
          }
          i <- i + 1
		  info <- sprintf("%d%% done", round(100*i/(N2 - 1)))
		  setTkProgressBar(pb, i, sprintf(" (%s)", info), info)
      }
	  close(pb)
  hourly <- cbind(hourly, CO_sd, Number, Other, Center)

  write.table(ExcelFormat(hourly),
              file = paste(site, "CO_Hourly_Average",
                           max(strptime(hourly[, 1], format = "%Y-%m-%d")),
...
datas <- COImporter("E:/1- Methane - Picarro/Raw Data/Madera/Madera")
datas <- DataFixer(datas, "Madera") 
hourly <- COHourlyAverage(datas, "Madera")
cat("
Finished averaging CO data for", "Madera 
", sep = " ")
